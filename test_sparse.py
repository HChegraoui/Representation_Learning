...
units = sess.run(encoded,feed_dict={inputs_:imgs})
####"
print("svm start")
print("SVM TRAIN")
clf = svm.SVC(kernel='rbf', C = 1)
clf.fit(units.reshape(len(units),-1),Y)
file = open("annotation_test2.txt","r")
X =[]
Y =[]
i = 0
x = 0
for line in file:
    if(i%100== 0  and i> 0):
        ...
        Y = np.asarray(Y)
        imgs = X.reshape((-1, 32, 32, 3))
        units_test = sess.run(encoded,feed_dict={inputs_:imgs})
    ...
    sl = line.split(' ')
    Y.append(int(sl[1][0]))
    img  = cv2.imread("output/test2/"+sl[0])/255
    X.append(img)
    i=i+1
print("test")
print(x)
